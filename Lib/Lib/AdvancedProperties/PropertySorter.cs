namespace Lib.AdvancedProperties
{
    ...
    public class PropertySorter : ExpandableObjectConverter
    {
        ...
        public AdvancedProperties GetEditableProperties(PropertyDescriptorCollection pdc, User usr=null, DisplayMode displayMode = DisplayMode.None)
        {
            var orderedProperties = new AdvancedProperties();
            foreach (PropertyDescriptor pd in pdc)
            {
                ...
                if (
                    ((db != null && (((Db)db).Ignore == null || ((Db)db).Ignore == false) && (((Db)db).Editable == null || ((Db)db).Editable == true)) || db == null)
                    &&
                    (access != null && (displayMode==DisplayMode.None || (access.DisplayMode & displayMode) == displayMode)
                    && (access.EditableFor == Permission.None.Value || usr==null || usr.HasAtLeastOnePermission(access.EditableFor)))
                    )
                {
                    orderedProperties.Add(GetProperty(pd));
                }
        ...
        private static AdvancedProperty GetProperty(PropertyDescriptor pd)
        {
            ...
            {
                ap.Validation.AlertMessage = ap.Common.DisplayName + " " + Tools.Utils.Translate.GetTranslatedValue("is_required", "FrontEnd");
            }
            if (ap.Common != null)
            {
