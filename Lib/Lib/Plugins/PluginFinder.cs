...
namespace Lib.Plugins
{
    ...
    public class PluginFinder : IPluginFinder
    {
        ...
        public virtual bool AuthenticateClient(PluginDescriptor pluginDescriptor, int storeId)
        {
            ...
            if (pluginDescriptor.LimitedToClients.Count == 0)
                return true;

            return pluginDescriptor.LimitedToClients.Contains(storeId);
        }
            ...
            foreach (var plugin in _plugins)
                if (typeof(T).IsAssignableFrom(plugin.PluginType))
                    if (!installedOnly || plugin.Installed)
                        if (AuthenticateClient(plugin, storeId))
