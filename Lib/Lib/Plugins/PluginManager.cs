    ...
    public class PluginManager
    {
        ...
        public static void Initialize()
        {
            using (new WriteLockDisposable(Locker))
            {
                ...
                var pluginFolder = new DirectoryInfo(MapPath(PluginsPath));
                _shadowCopyFolder = new DirectoryInfo(MapPath(ShadowCopyPath));
        ...
        {
            // allways installed on this step
           if (String.IsNullOrEmpty(systemName))
                throw new ArgumentNullException("systemName");

            var filePath = MapPath(InstalledPluginsFilePath);
        ...
        private static Assembly PerformFileDeploy(FileInfo plug)
        {
            ...
            if (CommonHelper.GetTrustLevel() != System.Web.AspNetHostingPermissionLevel.Unrestricted)
            {
            ...
            //AddApplicationPart(_mvcBuilder,shadowCopiedAssembly);
