...
namespace Lib.Plugins
{
    ...
    public static class PluginFileParser
    {
                    ...
                    case "LimitedToStores":
                        {
                                                      ...
                                                      .Select(x => x.Trim()))
                            {
                                ...
                                {
                                    descriptor.LimitedToClients.Add(storeId);
                                }
                            }
                        }
                        break;
        ...
        public static void SavePluginDescriptionFile(PluginDescriptor plugin)
        {
            ...
            if (plugin.LimitedToClients.Count > 0)
            {
                var storeList = "";
                for (int i = 0; i < plugin.LimitedToClients.Count; i++)
                {
                    storeList += plugin.LimitedToClients[i];
                    if (i != plugin.LimitedToClients.Count - 1)
                        storeList += ",";
                }
