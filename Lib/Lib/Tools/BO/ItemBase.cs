...
namespace Lib.Tools.BO
{
    ...
    [Serializable]
    [DataContract]
    [JsonConverter(typeof(ItemBasePropertyConverter))]
    public class ItemBase : IConvertible
    {
        ...
        /// </summary>
        [Db(_Populate = false, _Editable = false, _ReadableOnlyName = true)]
        ...
        [Db(_Editable = false, _Ignore = true)]
        [JsonIgnore]
        ...
        {
            return "LabControl/" + this.GetType().Name + "/" + GetId();
        }
        ...
        public virtual DataTable getAdditionalPrintInfo(ItemBase searchItem)
        {
            return null;
        }

        public virtual string PopulateReportWidget(ItemBase searchitem, string sSearch, ControllerContext ControllerContext, ViewDataDictionary ViewData, ITempDataDictionary TempData, User sUser = null, SqlConnection conn = null)
        {
            return "";
        }
        ...
        public virtual void Update(ItemBase item, DisplayMode DisplayMode = DisplayMode.Advanced, string Comment = "Updated", SqlConnection connection = null, User user = null)
        {
            ...
            var properties = pss.GetDbEditProperties(pdc, user, DisplayMode);
            ...
            if (tocontinue)
            {
                ...
                else
                {
                    ...
                    if (user == null)
                        user = Authentication.GetCurrentUser();//new User(Authentication.GetCurrentUserId());

                    dbparam = new SqlParameter("@UpdatedBy", SqlDbType.BigInt) { Value = user.Id };
                    lcommand.Parameters.Add(dbparam);
        ...
        public virtual Dictionary<long, ItemBase> PopulateReport(SqlConnection conn,
                                                                ...
                                                                List<SortParameter> SortParameters,
                                                                User sUser,
                                                                Lib.AdvancedProperties.DisplayMode mode,
                                                                out long itotal,
                                                                out long idisplaytotal)
        {
            return PopulateGeneric(conn,
                ...
                foreach (AdvancedProperty property in properties)
                {
                        ...
                        ) && IsLoadedFromDB(property.Type))
                    {
                        ...
                        else
                        {
                            ...
                            var addlSelectParam = sub_item.GetAdditionalSelectQuery(property);
                            parameterName += parameterName.Contains(addlSelectParam) ? "" : addlSelectParam;
        ...
        public virtual RequestResult SaveForm()
        {
            if (this.Id > 0)
            {
                ...
                return new RequestResult() { Result = RequestResultType.Reload, RedirectURL = URLHelper.GetUrl("DocControl/"+ this.GetType().Name + "/" + Id.ToString()), Message = "Saved successfully" };
            }
        ...
        public virtual Dictionary<string, object> LoadFrontEndViewdata()
        {
            ...
            ViewData.Add("ParentId", Convert.ToInt64(HttpContextHelper.Current.Request.Form["ParentId"]));
                ...
                foreach (AdvancedProperty property in properties)
                {
                    ...
                    if (property.Db.Readable==null || property.Db.Readable == true)
                    {
                        ...
                        else if (property.Type != typeof(Dictionary<long, ItemBase>))
                        {
                            ...
                            if (property.Type == typeof(NumbersRange))
                            {
                            ...
                            else if (property.Type == typeof(string) && value!=DBNull.Value && value!=null)
                            {
                                try
                                {
                                    ...
                                    var str = context.Session.GetString(SessionItems.Language);
                                    var pLanguage = JsonConvert.DeserializeObject<Language>(str);
                                    if (property.Translate != null && property.Translate.Translatable && pLanguage != null)
                                    {
                                        value = Translation.Load(property.PropertyName, this.GetType().Name, this.Id, pLanguage, value.ToString());
                                    }
