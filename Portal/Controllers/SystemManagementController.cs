// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SystemManagementController.cs" company="SecurityCRM">
//   Copyright 2020
// </copyright>
// <summary>
...
namespace SecurityCRM.Controllers
{
    ...
    {
        public SystemManagementController(ICompositeViewEngine viewEngine) : base(viewEngine)
        {
        }
        ...
        public ActionResult Edit(string BO, string Namespace, string BOLink = "ItemBase", string NamespaceLink = "Lib.BusinessObjects", string Id = "")
        {
            ...
            if (boproperties.AllowCreate && Lib.Tools.Security.Authentication.GetCurrentUser().HasAtLeastOnePermission(boproperties.CreateAccess))
            {
                ...
                var url = "SystemManagement/CreateItem/" + item.GetType().Name + "/" + System.Net.WebUtility.UrlEncode(item.GetType().Namespace);
                if (!string.IsNullOrEmpty(Id))
                {
        ...
        public ActionResult EditItem(string BO, string Namespace, long Id)
        {
            ...
            var item = Activator.CreateInstance(Type.GetType(Namespace + "." + BO + ", " + Namespace.Split('.')[0], true));
            ViewData["BOType"] = item.GetType();
            ViewData["Back_Link"] = Lib.Tools.Utils.URLHelper.GetUrl("SystemManagement/Edit/" + item.GetType().Name + "/" + System.Net.WebUtility.UrlEncode(item.GetType().Namespace));
            ...
            if (!string.IsNullOrEmpty(Request.Query["BOLink"]))
            {
                ...
                ViewData["Back_Link"] = Lib.Tools.Utils.URLHelper.GetUrl("SystemManagement/Edit/" + item.GetType().Name + "/" + System.Net.WebUtility.UrlEncode(item.GetType().Namespace) + "/" + BOLink + "/" + System.Net.WebUtility.UrlEncode(NamespaceLink) + "/" + IdLink.ToString());
            }
            else
            {
                ViewData["LinkItem"] = null;
        ...
        public async Task<ActionResult> Profile(string Id)
        {
            ...
            //HttpContext.Response.Cookies.Append("refreshToken", x.RefreshToken);
            if (!Authentication.CheckUser(this.HttpContext, Modulesenum.SMI))
            {
                return new RedirectResult(Config.GetConfigValue("SMILoginPage") + "?ReturnUrl=" + Request.GetEncodedPathAndQuery());
            }
            ...
            var NewPairTokens = await SecurityCRMAuthApiHelper.GetRequestAsync<AuthenticatedResponse>("Token/refresh", HttpContext);
            UpdateCookiesReffreshTokens(NewPairTokens.RefreshToken);
            //HttpContext.Response.Cookies.Delete("refreshToken");
            //HttpContext.Response.Cookies.Append("refreshToken", NewPairTokens.RefreshToken);
