    ...
    public class DataProcessor : IDataProcessor
    {
            ...
            try
            {
                ...
                if (!ReadOnly && property.Common.Editable == true && (property.Common.EditTemplate == EditTemplates.SelectList || property.Common.EditTemplate == EditTemplates.SelectListParent))
                {
                    ...
                    if (!ReadOnly)
                    {
                        ...
                        {
                            var url = URLHelper.GetUrl(Module.Substring(1, Module.Length - 2) + "/EditItem/" + slmodel.Value.GetType().Name + "/" + System.Net.WebUtility.UrlEncode(slmodel.Value.GetType().Namespace) + "/" + slmodel.Value.Id.ToString());
                ...
                else
                {
                    ...
                    if (property.Common.EditTemplate != EditTemplates.MultiSelect)
                    {
                        ...
                        if (!ReadOnly)
                        {
                            if (model.Value.Id > 0)
                            {
                                var url = URLHelper.GetUrl(Module.Substring(1, Module.Length - 2) + "/EditItem/" + model.Value.GetType().Name + "/" + System.Net.WebUtility.UrlEncode(model.Value.GetType().Namespace) + "/" + model.Value.Id.ToString());
        ...
        public object GetValue(AdvancedProperty property, string prefix = "", DisplayMode mode = DisplayMode.Simple)
        {
            ...
            try
            {
                ...
                {
                    var pId = HttpContextHelper.Current.Request.Form[prefix + property.PropertyName + "_id"];
                    if (!string.IsNullOrEmpty(pId))
                    {
                        long numid = 0;
                        if (!long.TryParse(pId, out numid) && !string.IsNullOrEmpty(HttpContextHelper.Current.Request.Form[prefix + property.PropertyName + "_id"]))
                        {
                            pId = HttpContextHelper.Current.Request.Form[prefix + property.PropertyName + "_id"];
                    ...
                    if (string.IsNullOrEmpty(pId) &&
                        string.IsNullOrEmpty(HttpContextHelper.Current.Request.Form[prefix + property.PropertyName]))
                    {
                        item = null;
                    }
                    else
                    {
                        item.SetName(HttpContextHelper.Current.Request.Form[prefix + property.PropertyName]);
                ...
                else if (!string.IsNullOrEmpty(HttpContextHelper.Current.Request.Form[prefix+property.PropertyName]) ||
                    !string.IsNullOrEmpty(HttpContextHelper.Current.Request.Form[prefix + property.PropertyName+"[]"]))
                {
                        ...
                        && (mode == DisplayMode.Search || mode == DisplayMode.PrintSearch))
                    {
                             ...
                             !string.IsNullOrEmpty(HttpContextHelper.Current.Request.Form[prefix + property.PropertyName + "[]"]))
                        {
                            ...
                            var ids = (!string.IsNullOrEmpty(HttpContextHelper.Current.Request.Form[prefix + property.PropertyName]) ? HttpContextHelper.Current.Request.Form[prefix + property.PropertyName].FirstOrDefault().Split(delim) : HttpContextHelper.Current.Request.Form[prefix + property.PropertyName + "[]"].FirstOrDefault().Split(delim));
                    ...
                    else if (property.Common.EditTemplate == EditTemplates.MultiSelect)
                    {
                        if (!string.IsNullOrEmpty(HttpContextHelper.Current.Request.Form[prefix + property.PropertyName]) ||
                            !string.IsNullOrEmpty(HttpContextHelper.Current.Request.Form[prefix + property.PropertyName + "[]"]))
                        {
                            var ids = (!string.IsNullOrEmpty(HttpContextHelper.Current.Request.Form[prefix + property.PropertyName]) ? HttpContextHelper.Current.Request.Form[prefix + property.PropertyName].FirstOrDefault().Split(',') : HttpContextHelper.Current.Request.Form[prefix + property.PropertyName+"[]"].FirstOrDefault().Split(','));
                            foreach (var sid in ids)
                            {
                                if (!string.IsNullOrEmpty(sid))
                                {
